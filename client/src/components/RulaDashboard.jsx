  ...
  const fetchDashboardData = async () => {
    ...
    try {
      const { data, error: fetchError } = await supabase
        .from('assessments')
        .select(`id, task_description, assessed_at, status, assessees (full_name, position), assessment_sides (side, final_score, action_level)`)
        .eq('status', 'completed')
  ...
  return (
    <div className="rula-dashboard dashboard-v2">
      ...
      <section className="assessment-panel panel">
        ...
        {loading ? <div className="dashboard-state"><span className="loading-ring" />กำลังเรียกดูข้อมูลการประเมิน...</div> : error ? <div className="dashboard-state dashboard-state--error">{error}<button onClick={fetchDashboardData}>ลองอีกครั้ง</button></div> : filteredAssessments.length === 0 ? <div className="dashboard-state"><Icon name="clipboard" size={28} /><b>ยังไม่มีรายการที่ตรงกับเงื่อนไข</b><span>ลองปรับคำค้นหาหรือตัวกรองความเสี่ยง</span></div> : <div className="table-responsive"><table className="assessment-table"><thead><tr><th>ผู้ถูกประเมิน / งาน</th><th>วันที่ประเมิน</th><th>คะแนนขวา</th><th>คะแนนซ้าย</th><th>สถานะความเสี่ยง</th></tr></thead><tbody>{filteredAssessments.map((item) => { const right = item.assessment_sides?.find((side) => side.side === 'right'); const left = item.assessment_sides?.find((side) => side.side === 'left'); const level = getHighestLevel(item); return <tr key={item.id}><td><div className="person-cell"><span className="person-avatar">{(item.assessees?.full_name || '?').slice(0, 1)}</span><div><b>{item.assessees?.full_name || 'ไม่ระบุชื่อ'}</b><small>{item.task_description || item.assessees?.position || 'ไม่ได้ระบุลักษณะงาน'}</small></div></div></td><td className="date-cell">{item.assessed_at ? new Intl.DateTimeFormat('th-TH', { day: 'numeric', month: 'short', year: 'numeric' }).format(new Date(item.assessed_at)) : '–'}</td><td><span className={`side-score score--${right?.action_level || 1}`}>{right?.final_score ?? '–'}</span></td><td><span className={`side-score score--${left?.action_level || 1}`}>{left?.final_score ?? '–'}</span></td><td><span className={`risk-badge risk-badge--${RISK[level].color}`}><i />ระดับ {level} · {RISK[level].label}</span></td></tr>; })}</tbody></table></div>}
      </section>
    </div>
  );
}
